...
class Active_Auditor_REST_Endpoints {
    ...
    public static function get_seo_endpoint($request) {
        ...
        $bust_cache = !empty($request->get_param('bust_cache'));
        $seo_data   = Active_Auditor_SEO_Scanner::scan_page($url, $bust_cache);
        ...
        $public_data = array(
            ...
            'canonical_tag'     => isset($seo_data['canonical_tag']) ? $seo_data['canonical_tag'] : array(),
            'og_tags'           => isset($seo_data['og_tags']) ? $seo_data['og_tags'] : array(),
            'structured_data'   => isset($seo_data['structured_data']) ? $seo_data['structured_data'] : array(),
            'readability'       => isset($seo_data['readability']) ? $seo_data['readability'] : array(),
        );
    ...
    public static function get_google_services_endpoint($request) {
        $url        = $request->get_param('url');
        $bust_cache = !empty($request->get_param('bust_cache'));
        $services   = Active_Auditor_Google_Services::detect_services($url, $bust_cache);
