...
class Addons extends CommonUtils\Addons {
	...
	public function getDownloadUrl( $sku ) {
		...
		$rawResponse = aioseo()->helpers->wpRemotePost( $this->getLicensingUrl() . 'addons/download-url/', [
			'timeout' => 30,
			'body'    => wp_json_encode( $payload )
		] );
		$response    = json_decode( wp_remote_retrieve_body( $rawResponse ) );

		if ( ! empty( $response ) && property_exists( $response, 'downloadUrl' ) && ! empty( $response->downloadUrl ) ) {
	...
	public function getMinimumVersion( $slug ) {
		$minimumVersions = [
			...
			'aioseo-link-assistant' => '1.1.12',
			'aioseo-local-business' => '1.3.12',
			'aioseo-news-sitemap'   => '1.0.19',
			'aioseo-rest-api'       => '1.0.9',
			'aioseo-video-sitemap'  => '1.1.25'
		];
