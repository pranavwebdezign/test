...
class License {
	...
	public function maybeValidate() {
		...
		if ( defined( 'AIOSEO_LICENSE_KEY' ) && ! empty( AIOSEO_LICENSE_KEY ) ) {
			...
			if ( AIOSEO_LICENSE_KEY !== $licenseKey ) {
				...
				aioseo()->core->cache->delete( 'license_last_checked' );
			}
		}
		...
		if ( aioseo()->core->cache->get( 'license_last_checked' ) ) {
			return;
		}
		...
		if ( $success || aioseo()->core->cache->get( 'failed_update' ) ) {
			...
			aioseo()->core->cache->update( 'license_last_checked', true, 12 * HOUR_IN_SECONDS );
		...
		aioseo()->core->cache->update( 'license_last_checked', true, HOUR_IN_SECONDS );
	}
	...
	private function activateHelper( $route, $key ) {
		...
		if ( $this->isActive() ) {
			aioseo()->seoChecklist->completeCheck( 'licenseActivated' );
		} else {
