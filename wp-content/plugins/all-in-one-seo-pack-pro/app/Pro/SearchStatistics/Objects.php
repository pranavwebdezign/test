...
class Objects {
	...
	/**
	 * Schedules the objects scan as a recurring action.
	 *
	 * @since   4.3.0
	 ...
	 * @return void
	 */
	private function scheduleScan() {
		// If we're in idle mode (no objects to scan), unschedule and don't reschedule yet.
		if ( aioseo()->core->cache->get( 'as_objects_scan_idle' ) ) {
		...
		aioseo()->actionScheduler->scheduleRecurrent( $this->action, 10, MINUTE_IN_SECONDS );
	}
	...
	public function reset() {
		...
		// Clear the idle cache so that the scan restarts immediately.
		aioseo()->core->cache->delete( 'as_objects_scan_idle' );
	...
	public function scan() {
		...
		$lockKey = 'as_objects_scan_running';
		...
		if ( empty( $objects ) ) {
			// No more objects to scan - set idle cache. The schedule method on the next init will unschedule.
			aioseo()->core->cache->update( 'as_objects_scan_idle', true, DAY_IN_SECONDS );
