...
class TermMeta {
	...
	public function importTermMeta() {
		...
		$timeStarted      = esc_sql( gmdate( 'Y-m-d H:i:s', aioseo()->core->cache->get( 'import_term_meta_rank_math' ) ) );
		...
		$terms = aioseo()->core->db
			...
			->whereLike( 'tm.meta_key', 'rank_math_%', true )
		...
		foreach ( $terms as $term ) {
			...
			if ( ! $termMeta || ! count( $termMeta ) ) {
				// Skip terms with no Rank Math meta (shouldn't happen with our query filter, but defensive check).
				continue;
			}
	...
	private function migrateRedirects( $term ) {
		...
		aioseo()->redirects->importExport->rankMath->importTermRedirect( $term->term_id );
	}
}