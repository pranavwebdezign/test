...
class Video {
	 ...
	 *
	 * @since   1.0.0
	 * @version 1.1.25 Remove is_admin() check to allow frontend scheduling and switch to recurring action.
	 *
	 * @return void
	 */
	...
	/**
	 * Schedules the video sitemap scan as a recurring action.
	 *
	 * @since   1.0.0
	 * @version 1.1.25 Switch to recurring action with cache-based idle state.
	 *
	 * @return void
	 */
	public function scheduleScan() {
		// If we're in idle mode (no posts/terms to scan), unschedule and don't reschedule yet.
		if ( aioseo()->core->cache->get( 'as_video_scan_idle' ) ) {
			aioseo()->actionScheduler->unschedule( $this->videoScanAction );

			return;
		}

		if ( aioseo()->actionScheduler->isScheduled( $this->videoScanAction ) ) {
			return;
		}

		$scanInterval = apply_filters( 'aioseo_video_sitemap_scan_interval', MINUTE_IN_SECONDS );
		aioseo()->actionScheduler->scheduleRecurrent( $this->videoScanAction, 10, $scanInterval );
	}
	 ...
	 *
	 * @since   1.0.0
	 * @version 1.1.25 Use recurring action with runtime lock and idle state.
	 *
	 * @return void
	 */
	public function scan() {
		// Runtime lock: Prevent concurrent execution of this action.
		$lockKey = 'as_video_scan_running';
		...
		if ( ! $postsFound && ! $termsFound ) {
			// No more posts/terms to scan - set idle cache. The schedule method on the next init will unschedule.
			aioseo()->core->cache->update( 'as_video_scan_idle', true, HOUR_IN_SECONDS );
		}
	...
	private function scanPosts() {
		...
		$postTypes    = (array) aioseo()->sitemap->helpers->includedPostTypes();
		...
		$posts = aioseo()->core->db
			...
			->whereRaw( '(
				...
				ap.video_scan_date IS NULL OR
				ap.video_scan_date < p.post_modified_gmt
			)' )
			->limit( $postsPerScan )
			->run()
			->result();
	...
	public function scanPost( $post = null ) {
		...
		$thirdPartyUrls = array_map( [ $this, 'prepareUrl' ], $this->thirdParty->getVideoUrls( $post ) );
	...
	private function customFields( $postId ) {
		...
		foreach ( $meta as $key => $value ) {
			// Exclude WordPress internal, AIOSEO, oEmbed, edit lock, Bricks, Elementor, and Oxygen meta.
			// Page builder content is handled separately by ThirdParty class to properly parse video elements.
			if (
				! preg_match( '#^(_wp.*|_aioseop_.*|_oembed_.*|_edit_.*|_bricks.*|_elementor.*|_oxygen.*)#', (string) $key ) &&
				! empty( $value[0] ) &&
				is_string( $value[0] )
			) {
